...
#ifndef SMOKEUNIT_SWS_H_
#define SMOKEUNIT_SWS_H_
...
HealthLevel = get HEALTH;
	if(HealthLevel < 33) {
		if(HealthLevel < 33 || HealthLevel >= 20) {
			sleep (RandomNumber - 200);
		}
		if(HealthLevel < 20 || HealthLevel >= 10) {
			sleep (RandomNumber - 250);
		}
		if(HealthLevel < 10 || HealthLevel >= 0) {
			sleep (RandomNumber - 300);
		}
		RollTheDice = Rand(0,10);
		if(RollTheDice > 0 || RollTheDice <= 1) {
			emit-sfx SMOKEPUFF_SWS from SMOKEPIECE1;
		}
		if(RollTheDice > 1 || RollTheDice <= 2) {
			emit-sfx SMOKEPUFF_SWS from SMOKEPIECE2;
		}
		if(RollTheDice > 2 || RollTheDice <= 3) {
			emit-sfx SMOKEPUFF_SWS from SMOKEPIECE3;
		}
		if(RollTheDice > 3 || RollTheDice <= 4) {
			emit-sfx SMOKEPUFF_SWS from SMOKEPIECE4;
		}
	}
}
}
#endif // if SMOKEUNIT_SWS_H_