...
-- user-configs
local _unitName     = 'reb_i_veterantrooper'  -- the saboteur
local _unitDefID    = UnitDefNames[_unitName].id
local _bombName     = 'reb_sp_bomb'  -- the unit which is created in the enemy unit and then selfd
local _countdown    = 5      -- detonation countdown in gamesec
local _plantTime    = 5      -- minimum seconds needed to plant the bomb
local _hpPTimeBonus = 0      -- _plantTime + _hpPTimeBonus == maximum time needed
local _hpPTimeMod   = 4000    -- it is part of a function (defined between 0-1) to adjust _hpPTimeBonus to the enemie's MaxHealth
local _damage       = 0  -- single unit damage additional to the bomb/mine selfd
...
local plantCmdDesc = {
  ...
  texture = "&.1x.1&unitpics/reb_sp_bomb.png&bitmaps/icons/frame.png",
  onlyTexture = true,
  params  = {}
}
