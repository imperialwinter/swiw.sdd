-- TREPAN'S ORIGINAL, UNMODIFIED CODE FOLLOWS
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
--
...
-- IMPERIAL WINTER SPECIFIC CODE FOLLOWS

for name,fd in pairs(FeatureDefs) do
	local energy = fd.energy or 0
	local metal = fd.metal or 0
	local reclaimtime = (energy + metal) / 13
	fd.reclaimtime = reclaimtime
	--Spring.Echo(name .. " set to " .. reclaimtime .. "\n")
end